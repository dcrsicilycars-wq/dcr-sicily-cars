...
VehicleData.prototype.loadVehicles = function() {
  var stored = localStorage.getItem('dcr-vehicles');
  if (stored) {
    try { this.vehicles = JSON.parse(stored); return Promise.resolve(this.vehicles); } catch(e) {}
  }
...
VehicleRenderer.prototype.createCard = function(vehicle) {
  ...
  return '<div class="car-card" data-id="' + vehicle.id + '\">'
    + '<div class="car-card-image"><img src="' + BASE_PATH + vehicle.image + '" alt="' + vehicle.brand + ' ' + vehicle.model + '" loading="lazy">' + badgeHtml + '</div>'
    ...
    + '<span class="car-card-spec"><strong>' + i18n.t('vehicle.mileage') + ':</strong> ' + vehicle.mileage.toLocaleString() + ' ' + i18n.t('vehicle.km') + '</span>'
